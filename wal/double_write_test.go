...
func TestDoubleWriteBuffer_WriteAndRecover(t *testing.T) {
	...
	err = dwb.WriteToJournal(testData, position)
	if err != nil {
		t.Fatalf("Failed to write to journal: %v", err)
	}
	
	// Mark as complete
	err = dwb.MarkComplete()
	if err != nil {
...
func TestDoubleWriteBuffer_RecoverIncomplete(t *testing.T) {
	...
	for _, entry := range entries {
		err = dwb.WriteToJournal(entry.data, entry.position)
		if err != nil {
...
func TestDoubleWriteBuffer_Clear(t *testing.T) {
	...
	// Write some data
	err = dwb.WriteToJournal([]byte("Test data"), 0)
	if err != nil {
...
func TestDoubleWriteBuffer_Compact(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		err = dwb.WriteToJournal(data, int64(i*100))
		if err != nil {
...
func TestDoubleWriteBuffer_CRCValidation(t *testing.T) {
	...
	err = dwb.WriteToJournal(testData, 0)
	if err != nil {
		t.Fatalf("Failed to write: %v", err)
	}
	journal.Close()
...
func TestDoubleWriteBuffer_LargeData(t *testing.T) {
	...
	err = dwb.WriteToJournal(largeData, 0)
	if err != nil {
...
func BenchmarkDoubleWriteBuffer_Write(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		err = dwb.WriteToJournal(testData, int64(i*1024))
		if err != nil {
